...
# Порог уверенности, ниже которого ретриверу не доверяем и эскалируем.
# 0.55 — баланс между «слишком охотно эскалирует» (было 0.6) и «тянет
# нерелевантный контекст» (было бы <0.5).
ESCALATION_SCORE_THRESHOLD = 0.55
...
def _is_escalated(top_score: float, answer: str) -> bool:
    ...
    Эскалируем если:
    - top_score ниже порога доверия ретриверу, ИЛИ
    - модель явно вернула отказ как основное содержание ответа
      (короткий ответ: «НУЖЕН_СПЕЦИАЛИСТ», «не знаю», «нет информации»).
    Длинный полезный ответ с упоминанием «нужен специалист» в конце —
    НЕ эскалация.
    """
    if top_score < ESCALATION_SCORE_THRESHOLD:
        return True
    ...
    short = len(a) < 80
    if short and ("не знаю" in a or "нужен специалист" in a):
        return True
    if short and any(m in a for m in _NO_INFO_MARKERS):
        return True
...
_SERVICE_RULES = [
    # Специфичные темы идут раньше «IT-инфраструктуры», иначе общие слова
    # вроде «ноутбук»/«пк» перехватывают запросы про почту/1С/принтеры.
...
# TaskType — тип обращения по классификации IntraService / ITIL:
# Инцидент           — что-то сломалось, отказ сервиса
# Запрос             — нужен доступ, оборудование, установка
# Консультация       — «как сделать», «подскажите», вопрос без сбоя
# Проблема           — повторяющийся инцидент, системная причина
_TASKTYPE_RULES = [
    ("Проблема",     ["постоянно", "каждый день", "каждый раз", "всегда", "повторяется", "регулярно", "снова и снова"]),
    ("Инцидент",     ["не работает", "не запускается", "не открывается", "не печатает", "не подключается",
                      "упал", "сломал", "отказ", "завис", "зависает", "выдаёт ошибку", "выдает ошибку",
                      "ошибк", "перестал", "пропал", "не приходит", "не отправляет"]),
    ("Запрос",       ["нужен доступ", "предоставить доступ", "дайте доступ", "заведите",
                      "создать учётную", "создать учетную", "новый сотрудник", "новая учётка",
                      "установить", "поставить программу", "нужна лицензия", "купить",
                      "выдать", "заменить", "подключить принтер", "настроить новый"]),
    ("Консультация", ["как ", "где ", "можно ли", "подскажите", "узнать", "объясните",
                      "консультация", "инструкция", "вопрос", "помогите разобраться"]),
]
...
def classify(user_query: str) -> dict:
    ...
    Возвращает service / task_type / priority — три поля из IntraService.
    """
    q = user_query.lower()
    service = "Другое"
    ...
    for pri, keywords in _PRIORITY_RULES:
        if any(kw in q for kw in keywords):
            priority = pri
            break
    task_type = "Инцидент"  # default — большинство обращений это сбои
    for tt, keywords in _TASKTYPE_RULES:
        if any(kw in q for kw in keywords):
            task_type = tt
            break
    return {"service": service, "task_type": task_type, "priority": priority}
