...
ВАЖНО: Если контекст касается похожей, но другой темы — НЕ натягивай его на вопрос. Лучше честно признай, что точного ответа нет.
Если вопрос специфичен и контекст не покрывает его напрямую — ответь строго: «НУЖЕН_СПЕЦИАЛИСТ» и больше ничего.
Если контекст точно совпадает с вопросом — дай краткий ответ по пунктам.
Никогда не додумывай и не смешивай несвязанные темы. Отвечай на русском языке."""
...
def _is_escalated(top_score: float, answer: str) -> bool:
    a = answer.lower()
    return top_score < 0.6 or "не знаю" in a or any(m in a for m in _NO_INFO_MARKERS)
